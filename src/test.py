...
def run_test(model_dir):
    ...
    for eval_task in eval_task_names:
        ...
        if eval_task == 'firstlevel':
            testLabels = ['RS', 'S', 'Q', 'A', 'PF', 'ACK', 'RF', 'NF', 'C', 'H', 'DIR', 'RC', 'LF']
            # testLabels = ['A', 'ACK', 'C', 'DIR', 'H', 'LF', 'NF', 'PF', 'Q', 'RC', 'RF', 'S']
        elif eval_task == 'secondlevel':
            testLabels = ['ACK', 'AEX', 'AR', 'AWH', 'AYN', 'C', 'DIR', 'E', 'FNU', 'FU',
                      'GRE', 'HI', 'I', 'LF', 'NF', 'O', 'OEX', 'PF', 'QD', 'QEX', 'QF',
                      'QI', 'QO', 'QP', 'QQ', 'QR', 'R', 'RC', 'RF', 'RFI']


        predLabels = [testLabels[x] for x in preds]


        np.seterr(divide='ignore', invalid='ignore')
        result = compute_metrics(eval_task, preds, out_label_ids)
        np.seterr(divide='warn', invalid='warn')
        acc = str(result['acc']).split('.')[1][0:6]
        results.update(result)

        print(predLabels)
        df = pd.DataFrame({'label': predLabels})
        df.to_csv(eval_task + '_' + "pred_" + seedNum + '_' + acc + ".csv", index=False)
...
for subdir, dirs, files in os.walk(args.models):
    ...
    elif 'test_results.txt' in files:
        print("test_results.txt has already been processed. Please remove it if you want to generate the new one.")
